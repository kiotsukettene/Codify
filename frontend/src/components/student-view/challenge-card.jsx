...
const StudentChallengeCard = ({ id, title, description, tags = [], status = 'pending' }) => {
  ...
  return (
    <Card className="w-full h-[23rem] mx-auto bg-white rounded-xl shadow-sm border-none overflow-hidden transition-all duration-300 dark:bg-gray-950 p-5">
    {/* Image Section */}
    <div className="relative bg-[#EEF7FE] rounded-lg">
      ...
      <CardContent className="mt-6 space-y-2 p-0 flex-grow">
        ...
        <h4 className="text-sm overflow-hidden text-ellipsis line-clamp-2 max-h-12 text-neutral-900 dark:text-neutral-200">{description}</h4>
      </CardContent>

      <CardFooter className="p-0 mt-5 justify-end">
